...
func (colorVariation ColorVariation) ColorSizeInfo() string {
	db.DB.First(&colorVariation.Color, colorVariation.ColorID)
	db.DB.Model(&colorVariation).Related(&colorVariation.SizeVariations)
	result := fmt.Sprintf("%s(", colorVariation.Color.Name)
	for i, v := range colorVariation.SizeVariations {
		db.DB.Model(&v).Related(&v.Size)
		if i < len(colorVariation.SizeVariations) - 1 {
			result = fmt.Sprintf("%s%s#%d, ", result, v.Size.Name, v.AvailableQuantity)
		} else {
			result = fmt.Sprintf("%s%s#%d)", result, v.Size.Name, v.AvailableQuantity)
		}
	}
